...
static int inpub_id;
static void mqtt_incoming_publish_cb(void *arg, const char *topic, u32_t tot_len) {
//	sprintf(bufmqtt, "Incoming publish at topic %s with total length %u\n\r", topic, (unsigned int) tot_len);
//	HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt), 1000);
	/* Decode topic string into a user defined reference */
	if (strcmp(topic, SetSettings.mqtt_tpc) == 0) {
		inpub_id = 0;
	} else if (topic[0] == 'A') {
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len,u8_t flags) {
//	sprintf(bufmqtt, "Incoming publish payload with length %d, flags %u\n\r",len, (unsigned int) flags);
//	HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt), 1000);
	...
	if (flags & MQTT_DATA_FLAG_LAST) {
		...
		//if (inpub_id != 0) {
			...
			//if (data[len - 1] != 0) {
//				sprintf(bufmqtt, "mqtt_incoming_data_cb: %s\n\r",(const char*) data);
//				HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt),1000);
			} else {
//				sprintf(bufmqtt,"mqtt_incoming_data_cb: Ignoring payload...\n\r");
//				HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt),1000);
			}
		}
//	}
...
//	sprintf(bufmqtt, "Subscribe result: %d\n\r", result);
//	HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt), 1000);
...
//		sprintf(bufmqtt, "mqtt_connection_cb: Successfully connected\n");
//		HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt), 1000);
		...
		err = mqtt_subscribe(client, SetSettings.mqtt_tpc, 0, mqtt_sub_request_cb, arg);

		if (err != ERR_OK) {
//			sprintf(bufmqtt, "mqtt_subscribe return: %d\n", err);
//			HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt), 1000);

		}
	} else {
//		sprintf(bufmqtt, "mqtt_connection_cb: Disconnected, reason: %d\n",status);
//		HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt), 1000);

		/* Its more nice to be connected, so try to reconnect */
		example_do_connect(client, SetSettings.mqtt_tpc);
	}
...
//  err = mqtt_client_connect(client, &mqttServerIP, MQTT_PORT, mqtt_connection_cb, 0, &ci);
	...
	if (err == ERR_ISCONN)
	{
		//sprintf(bufmqtt, "MQTT connection already established. %d\n\r", err);
		//HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt), 1000);
	} else if ((err != ERR_OK) && (err != ERR_ISCONN))
	{/* For now just print the result code if something goes wrong */
//		sprintf(bufmqtt, "mqtt_connect return %d\n\r", err);
//		HAL_UART_Transmit(&huart3, (uint8_t*) bufmqtt, strlen(bufmqtt), 1000);
	} else {
