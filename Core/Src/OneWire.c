...
uint16_t USART_ReceiveData(USART_TypeDef* USARTx)
{
  ...
  /* Receive Data */
  return (uint16_t)(USARTx->RDR & (uint16_t)0x01FF);// zerg
}
...
void USART_SendData(USART_TypeDef* USARTx, uint16_t Data)
{
  ...
  /* Transmit Data */
  USARTx->TDR = (Data & (uint16_t)0x01FF);// zerg
}
...
void owReadHandler() { //обработчик прерывания USART
  ...
  if (((OW_USART->CR1 & USART_CR1_RXNEIE) != 0) &&
      ((OW_USART->ISR & UART_FLAG_RXNE) != (uint16_t)RESET)) {// zerg

    /* Получаем данные из периферии и сбрасываем флаг*/
		while ((OW_USART->ISR & UART_FLAG_RXNE) == (uint16_t)RESET){;}// zerg
